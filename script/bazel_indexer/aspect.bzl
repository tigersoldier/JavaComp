...
#
# Aspect for javacomp, taken from intellij_info.bzl
...
def _aspect_impl(target, ctx):
  ...
  if hasattr(target, "java"):
    ...
    output = ctx.new_file(target.label.name + ".javacomp-build.json")
...
javacomp_aspect = aspect(implementation = _aspect_impl,
    attr_aspects = DEPENDENCY_ATTRIBUTES
)
"""Aspect for JavaComp plugin.

This aspect produces information for integration with JavaComp. This only
produces information for Java targets.

This aspect has two output groups:
  - ide-info-text produces .javacomp-build.json files that contains information
...
An javacomp-build.json file is a json blob with the following keys:
```javascript
{
